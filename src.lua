        ...
        do
            ...
            function window.dropdown:Refresh()
                if self.selected ~= nil then
                    ...
                    -- Refresh values if it's a function
                    if list.valuesFunction then
                        list.values = list.valuesFunction();
                    end
        ...
        function window:AddTab(text, order)
            ...
            function tab:AddSection(text, side, order)
                ...
                function section:AddList(data)
                    ...
                    if typeof(list.values) == 'function' then
                        list.valuesFunction = list.values;
                        list.values = list.valuesFunction();
                    end
                    ...
                    function list:AddValue(value)
                        if list.valuesFunction then
                            -- For function-based values, we can't directly modify the values
                            -- The function should handle adding values internally
                            error("Cannot add values to function-based dropdown. Modify the function instead.");
                        else
                            table.insert(list.values, tostring(value));
                            if window.dropdown.selected == list then
                                window.dropdown:Refresh()
                    ...
                    function list:RemoveValue(value)
                        if list.valuesFunction then
                            -- For function-based values, we can't directly modify the values
                            -- The function should handle removing values internally
                            error("Cannot remove values from function-based dropdown. Modify the function instead.");
                        else
                            if table.find(list.values, value) then
                                table.remove(list.values, table.find(list.values, value));
                                if window.dropdown.selected == list then
                    ...
                    function list:ClearValues()
                        if list.valuesFunction then
                            -- For function-based values, we can't directly modify the values
                            -- The function should handle clearing values internally
                            error("Cannot clear values from function-based dropdown. Modify the function instead.");
                        else
                            table.clear(list.values);
                            if window.dropdown.selected == list then
                                window.dropdown:Refresh()
